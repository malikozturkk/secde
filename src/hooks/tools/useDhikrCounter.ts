...
const revive = (raw: unknown): DhikrState | null => {
  ...
  const presetId =
    ...
    DHIKR_PRESETS.some((p) => p.id === v.presetId)
      ? v.presetId
      : DEFAULT_STATE.presetId;
  return {
    presetId,
    count:
      typeof v.count === "number" && v.count >= 0 ? Math.floor(v.count) : 0,
    completedRounds:
...
const store = createPersistentStore<DhikrState>(
  DHIKR_STORAGE_KEY,
  DEFAULT_STATE,
  revive
);
...
export const useDhikrCounter = (): DhikrCounter => {
  ...
  const increment = useCallback(() => {
    store.set((current) => {
      const activePreset =
        DHIKR_PRESETS.find((p) => p.id === current.presetId) ??
        DHIKR_PRESETS[0]!;
  ...
  const resetCount = useCallback(() => {
    store.set((current) =>
      current.count === 0 ? current : { ...current, count: 0 }
    );
  }, []);
  ...
  return {
    ...
    progressPercent:
      preset.target > 0
        ? Math.min(100, Math.round((state.count / preset.target) * 100))
        : 0,
