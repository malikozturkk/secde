...
export default function GuideRunnerPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
    ...
    return (
      <div className="fixed inset-0 bg-[#0b1215] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-white/40 font-black uppercase tracking-widest text-[13px]">İçerik Yükleniyor...</p>
        </div>
      </div>
    );
  }
  ...
  if (error || !data) {
    return (
      <div className="fixed inset-0 bg-[#0b1215] flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-white/60 mb-8 max-w-[400px]">Rehber yüklenirken bir sorunla karşılaştık. Lütfen daha sonra tekrar deneyin.</p>
        <Link href="/learn" className="bg-[#1a3842] text-[#4fc3f7] px-8 py-4 rounded-2xl font-black uppercase tracking-wide hover:bg-[#234b58] transition-colors">
          Kütüphaneye Dön
        </Link>
      </div>
    );
      ...
      <div className="fixed inset-0 bg-[#0b1215] z-[60] flex flex-col items-center justify-center p-6 text-center">
        ...
        >
          ...
          <p className="text-[18px] font-bold text-white/50 max-w-[360px] mx-auto leading-relaxed italic">
            <span className="text-white/90">{guide.title}</span> rehberini başarıyla tamamladın. Namazına ve ibadetine kabul olsun.
          </p>
        </motion.div>
  ...
  return (
    <div className="fixed inset-0 bg-[#0b1215] z-50 flex flex-col overflow-hidden">
      ...
      <div className="w-full max-w-[800px] mx-auto pt-8 px-6">
        <ProgressBar progress={progress} onClose={() => router.push("/learn")} accentColor={accentColor} />
      </div>
        ...
        <div className="w-full max-w-[900px] py-10">
          <AnimatePresence mode="wait" custom={direction}>
            <motion.div
              ...
              className="w-full"
            >
              <StepContent step={currentStep} isNamaz={isNamaz} accentColor={accentColor} />
            </motion.div>
          </AnimatePresence>
        </div>
      </div>
